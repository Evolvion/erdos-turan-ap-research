...
% decoupled from the Taylor & Francis `interact` class.
...
\begin{theorem}[Conditional window theorem; STRUCT]\label{thm:window}
\textbf{Assumptions.} For fixed $\sigma>0$ the instance satisfies
(i) $\max_{q\le M^{1/3}} \max_a |\widehat g(a;q)| \le (\log M)^{-1-\sigma}$,
(ii) $\sum_{t=1}^{k-1}\delta_t \le (\log M)^{-1-\sigma}$,
(iii) $\eta_k \le (\log M)^{-1-\sigma}$.
\textbf{Conclusion.} There exists $C_k(\sigma)$ such that if
$\mu \ge C_k(\sigma)(\log M)^{-1-\sigma}$, then $A\cap I$ contains a $k$--AP.
\end{theorem}
...
\caption{Schematic overview of the ET--AP engine (balancing, testing, and zooming phases).}
\label{fig:engine}
\end{figure}
\begin{algorithm}[H]
