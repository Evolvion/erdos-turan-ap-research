...
\small MSC 2020: Primary 05D10; Secondary 11B25, 68R05}
...
\begin{enumerate}[leftmargin=2em]
  ...
  \item \emph{Window theorem and ET--AP.} A robust window theorem (Theorem~\ref{thm:window}) and its STRUCT specialization (Corollary~\ref{cor:struct}), combined with the dyadic frequency lemma (Lemma~\ref{lem:dyadic-frequency}), yield a conditional implication of ET--AP.
\end{enumerate}

\paragraph{STRUCT assumptions.}
By STRUCT we mean that, on infinitely many dyadic windows $I$, the calibrated majorant $\tilde\nu$ has small low-modulus Fourier mass, small $t$-wise codegree deviations, and small linear-forms error; see Section~\ref{sec:struct} for the precise formulation used in Theorem~\ref{thm:window} and Corollary~\ref{cor:struct}.
...
$w\in\{\nu,\tilde\nu\}$ & Choice of weight in $T_3$ (Theorem~\ref{thm:sqrt}) \\
...
\begin{lemma}[Dyadic frequency lemma]\label{lem:dyadic-frequency}
Let $A\subset\mathbb{N}$ with $\sum_{n\in A} 1/n = \infty$. For each integer $j\ge 0$ let $I_j=[2^j,2^{j+1})$ and let $\tilde\nu_j$ be the harmonic weight on $I_j$, and set
\[
\mu_j \coloneqq \mathbb{E}_{\tilde\nu_j} 1_{A\cap I_j}.
\]
If $(\tau(j))_{j\ge 0}$ is a sequence of positive reals with $\sum_{j=0}^\infty \tau(j)<\infty$, then the set
\[
\{\,j : \mu_j \ge \tau(j)\,\}
\]
is infinite.
...
By definition,
\[
\sum_{j\ge 0} \mu_j H(I_j) = \sum_{n\in A} \frac{1}{n} = \infty,
\]
and $H(I_j)\asymp 1$ uniformly in $j$. If $\mu_j < \tau(j)$ for all but finitely many $j$, then for some absolute constant $C>0$ we would have
\[
\sum_{j\ge 0} \mu_j H(I_j) \le C \sum_{j\ge 0} \tau(j) < \infty,
\]
a contradiction. Hence infinitely many $j$ satisfy $\mu_j \ge \tau(j)$.
\end{proof}
...
These robust inequalities follow from the weighted CS--Gowers estimate (Proposition~\ref{prop:weighted-CSG}) and the generalized von Neumann lemma (Lemma~\ref{lem:gvn}) applied to the $k$--AP linear systems; detailed derivations are given in Section~\ref{sec:proofs}.

\begin{theorem}[Square--root pruning]\label{thm:sqrt}
Let $T_3=T_{3,w}(f)$ for $w\in\{\nu,\tilde\nu\}$. There is a constant $C_k>0$, depending only on $k$ and the fixed structural parameters, and a deletion set of $w$--mass $\rho\le 2\sqrt{T_3}+C_k(\delta+\eta)$ that makes the remainder $3$--AP--free.
\end{theorem}

\begin{theorem}[Robust window theorem]\label{thm:window}
Let $I=[M,2M)$ and let $f=1_A$. Write $\mu=\mathbb{E}_{\tilde\nu} f$, $r_{k-1}=\|f-\mu\|_{U^{k-1}(\tilde\nu)}$, and set
\[
\Delta \coloneqq \sum_{t=1}^{k-1}\delta_t + \eta_k.
\]
There exists a constant $C_k>0$, depending only on $k$ and the fixed structural parameters, such that if
\[
\mu \ge C_k\bigl(r_{k-1} + \Delta^{1/k}\bigr),
\]
then $A\cap I$ contains a $k$--term arithmetic progression.
\end{theorem}

\begin{corollary}[STRUCT specialization]\label{cor:struct}
Assume STRUCT as in Section~\ref{sec:struct}. Suppose in addition that
\[
r_{k-1} \le \varepsilon(M),\qquad \sum_{t=1}^{k-1}\delta_t \le \varepsilon(M),\qquad \eta_k \le \varepsilon(M),
\]
with $\varepsilon(M)=(\log M)^{-1-\sigma}$. Then there exists a constant $C_k(\sigma)>0$, depending only on $k$ and $\sigma$, such that if
\[
\mu \ge C_k(\sigma)(\log M)^{-1-\sigma},
\]
the set $A\cap I$ contains a $k$--term arithmetic progression.
\end{corollary}
...
We require: (i) low-modulus Fourier mass $\max_{q\le M^{1/3}}\max_a |\widehat g(a;q)|<\varepsilon(M)$, (ii) degree deviations $\sum_t\delta_t\le \bar\delta(M)$, and (iii) calibrated LFC error $\eta(M)$. We take all three scales equal, setting $\varepsilon(M)=\bar\delta(M)=\eta(M)=(\log M)^{-1-\sigma}$. These STRUCT conditions are exactly those invoked in the robust window theorem and its specialization (Theorem~\ref{thm:window} and Corollary~\ref{cor:struct}).
...
Including the structural error from the learned majorant (where degrees are approximate) adds a term $C_k(\delta+\eta)$, where $C_k$ depends only on $k$ and the bounded number of pruning stages.
\qed
